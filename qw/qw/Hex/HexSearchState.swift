...
final class HexSearchState: ObservableObject {
    ...
    func findMatches(in data: Data) {
        ...
        guard let pattern = patternBytes() else {
            matches = []
            currentMatchIndex = 0
            return
        }
        ...
        while offset + patternCount <= count {
            ...
            if found {
                ...
                offset += patternCount // non-overlapping
            } else {
                offset += 1
            }
        }
    ...
    func replace(in hexDocument: HexDocument) {
        ...
        guard let replacement = replacementBytes() else { return }

        let match = matches[currentMatchIndex]
        applyReplacement(in: hexDocument, range: match, replacement: replacement)
        findMatches(in: hexDocument.currentData)
    }
